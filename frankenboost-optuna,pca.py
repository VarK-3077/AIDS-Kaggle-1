...
def get_model(X_train, y_train):
    """
    Builds and returns a FrankenBoost model where the XGBoost core
    has been tuned using Optuna.
    """
    ...
    def objective(trial):
        ...
        params = {
            ...
            'gamma': 0, # Keep the user's no-pruning rule
            'colsample_bylevel': np.sqrt(X_train.shape[1]) / X_train.shape[1], # Keep the random feature rule
    ...
    print("Starting hyperparameter tuning for the XGBoost core...")
    ...
    # --- 3. Build the Final Tuned FrankenBoost Model ---
    
    # Get the fixed parameters from the original design
    # Note: X_train.shape[1] will now be the number of principal components
    k_features_ratio = np.sqrt(X_train.shape[1]) / X_train.shape[1]
    
    # Create the tuned XGBoost core with the best parameters found
    tuned_xgb_core = xgb.XGBClassifier(
        gamma=0,
    ...
    # Create the final FrankenBoost model with the tuned core
    frankenboost_model = BaggingClassifier(
        estimator=tuned_xgb_core,
        n_estimators=50,
        bootstrap=True,
        n_jobs=-1,
        random_state=42
...
# This is now required because StandardScaler and PCA cannot handle NaNs.
...
# --- 5. Build the FrankenBoost Model ---
print("Building the custom FrankenBoost model (with Optuna tuning) on PCA features...")
# The get_model function now runs the tuning process on the PCA-transformed data
frankenboost_model = get_model(X_pca, y_train)
print("Model built successfully.")
print("-" * 30)

# --- 6. Evaluate the Model with 10-Fold Cross-Validation ---
print("Evaluating the final tuned FrankenBoost model using 10-fold cross-validation...")
...
roc_auc_scores = cross_val_score(frankenboost_model, X_pca, y_train, cv=kfold, scoring='roc_auc')
accuracy_scores = cross_val_score(frankenboost_model, X_pca, y_train, cv=kfold, scoring='accuracy')

print("-" * 30)
print("Tuned FrankenBoost Model Evaluation Complete:")
